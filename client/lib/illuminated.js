...
(function (cp) {
  ...
  **/
    cp.Light.prototype._getVisibleMaskCache = function () {
        ...
        if (this.vismaskhash != hash) {
            ...
            const c = (this._vismaskcache = createCanvasAnd2dContext(
                `vm${  this.id}`,
                2 * d,
                2 * d
            ));
  ...
  **/
    cp.Light.prototype.bounds = function () {
        const orientationCenter = new cp.Vec2(
            Math.cos(this.angle),
            -Math.sin(this.angle)
  ...
  **/
    cp.Lamp.prototype.forEachSample = function (f) {
        ...
        for (let s = 0, l = this.samples; s < l; ++s) {
            ...
            const delta = new cp.Vec2(Math.cos(a) * r, Math.sin(a) * r);
  ...
  */
    ...
    function getTan2(radius, center) {
        const epsilon = getTan2.epsilon || 1e-6; // constant
            let x0;
            let y0;
            let len2;
            let soln;
    ...
    cp.PolygonObject.prototype.cast = function (ctx, origin, bounds) {
        ...
        const distance =
            (bounds.bottomright.x -
                ...
                (bounds.bottomright.y - bounds.topleft.y)) /
            2;
        this._forEachVisibleEdges(origin, bounds, function (
            a,
            b,
            originToA,
            originToB,
    ...
    cp.PolygonObject.prototype._forEachVisibleEdges = function (
        ...
        for (let p = 0, l = this.points.length; p < l; ++p, a = b) {
            ...
            if (a.inBound(bounds.topleft, bounds.bottomright)) {
                ...
                const aToB = b.sub(a);
                const normal = new cp.Vec2(aToB.y, -aToB.x);
  ...
  **/
    cp.RectangleObject.prototype.syncFromTopleftBottomright = function () {
        ...
        const b = new cp.Vec2(this.bottomright.x, this.topleft.y);
        const c = this.bottomright;
        const d = new cp.Vec2(this.topleft.x, this.bottomright.y);
        this.points = [a, b, c, d];
    };
  ...
  **/
    const getRGBA = (cp.getRGBA = (function () {
  ...
  **/
    const extractColorAndAlpha = (cp.extractColorAndAlpha = (function () {
        // var ctx = createCanvasAnd2dContext('grgba', 1, 1);
        const canvas = document.createElement('canvas');
        canvas.width = canvas.height = 1;
        const ctx = canvas.getContext('2d');
  ...
  **/
    function extend(extending /* , arg1, arg2, ... */) {
        for (let a = 1, l = arguments.length; a < l; ++a) {
            const source = arguments[a];
            if (source)
